...
def get_new_dump(train_command, i, delete_old) :
    ...
    if not os.path.isdir(new_dump_path) :
        os.mkdirs(new_dump_path)
    
    #Changing checkpoint to load model from. 
    dump_dir_dict = [f.path for f in os.scandir(path) if f.is_dir()]
    dump_dirs.sort()
    checkpoint_path = os.path.join(dump_dirs[0], 'checkpoint.pth')
    assert os.path.isfile(checkpoint_path)
    replace_arg(train_command, checkpoint_path, '--reload_model')
...
praser.add_argument('--delete_old', action='store_true', help='If this flag is provided, all folders in --dump_path except the one for the first GPU , are deleted for all the previous runs.')
...
for j in range(args.n_reps) :
    ...
    while True :
        ...
        split_file_dir = os.path.join(original_dp, 'split'+str(i+j*tot_splits))
